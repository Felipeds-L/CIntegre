...
describe('SchoolController', () => {
  ...
  beforeEach(() => {
    ...
    schoolServiceMock =
      new SchoolService() as jest.Mocked<SchoolService>;
  ...
  it('Should create a School', async () => {
    ...
    schoolServiceMock.createSchool.mockResolvedValue(
      mockSchool,
    );

    await schoolController.createSchool(
      req as Request,
      res as Response,
    );
    expect(
      schoolServiceMock.createSchool,
  ...
  it('Should return 200 if it can looking for a school', async () => {
    ...
    schoolServiceMock.getSchool.mockResolvedValue(
      mockSchool,
    );

    await schoolController.getSchool(
      req as Request,
      res as Response,
    );
    expect(
      schoolServiceMock.getSchool,
  ...
  it('Should return 200 if all Schools are found', async () => {
    ...
    schoolServiceMock.getAllSchools.mockResolvedValue(
      mockSchool,
    );
  ...
  it('Should return 500 if it throws', async () => {
    schoolServiceMock.getAllSchools.mockRejectedValue(
      new Error('Internal Error Service'),
    );
  ...
  it('Should return 200 if it School is updated', async () => {
    ...
    schoolServiceMock.updateSchool.mockResolvedValue(
      mockSchool,
    );
    ...
    expect(
      schoolServiceMock.updateSchool,
  ...
  it('Should return 404 if school to update is not found', async () => {
    ...
    expect(
      schoolServiceMock.updateSchool,
    ).toHaveBeenCalledWith(99, req.body);
    expect(res.status).toHaveBeenCalledWith(404);
    expect(res.json).toHaveBeenCalledWith({
      error: 'School not found',
    });
  ...
  it('Should return 200 if school is deleted', async () => {
    ...
    schoolServiceMock.deleteSchool.mockResolvedValue(
      mockSchool,
    );

    await schoolController.deleteSchool(
      req as Request,
      res as Response,
    );

    expect(
      schoolServiceMock.deleteSchool,
    ).toHaveBeenCalledWith(1);
  ...
  it('Should return 500 if delete throws', async () => {
    req.params = { id: '99' };
    schoolServiceMock.deleteSchool.mockRejectedValue(
      new Error('Internal Error Service'),
    );
